...
## 쉬운 실행 도구 — kafka-01-v1.1

2026-09-13, 동일한 macOS ARM64 환경에서 확인했습니다.

- `bash -n play Kafka-01.command` 통과.
- `./play verify`: JAVA_HOME을 명령에 따로 지정하지 않아도 설치된 JDK 21을 선택하고 기존 7개 시나리오 통과.
- 저장소 밖에서 `Kafka-01.command`를 실행해 번호 메뉴, 잘못된 읽기 개수(9)의 재입력 안내, COUNT=1의 안내 실습 6단계, 종료 안내와 종료 코드 0을 확인.
- COUNT=1에서 `[0]/committed=null` → `[0]/committed=1` → `[1,2]/committed=3`을 확인. 다른 그룹·seek·새 Consumer의 결과도 확인.
- `./play guided`에서 첫 실험 전에 `q`를 입력하여 정상 종료(코드 0).
- `./play invalid`는 사용법을 출력하고 종료 코드 2로 종료.
- 실행 파일 자체와 대화형 흐름은 터미널에서 검증했습니다. Finder의 파일 열기 이후 Terminal 앱 화면은 자동화 도구의 접근 제한으로 검증하지 못했습니다.

최초 글과 코드의 `kafka-01-v1` 태그는 변경하지 않았습니다.
